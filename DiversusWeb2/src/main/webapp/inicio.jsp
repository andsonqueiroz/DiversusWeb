...
<head>
    ...
    <title>Início</title>
    <link rel="stylesheet" href="css/styleinicioIndex.css">
            ...
            <%for(int i = posts.size()-1; i >=0 ; i--){ %>
                <div class="post">
                    ...
                    <%if(EspecialistaDao.buscarEspecialistaPorId(posts.get(i).getIduser()).getCaminhoImg() != null){ %>
                       <a href="RedirecionarServlet?action=perfil&id=<%= EspecialistaDao.buscarEspecialistaPorId(posts.get(i).getIduser()).getId()%>"> <img src="FotosUser/<%= EspecialistaDao.buscarEspecialistaPorId(posts.get(i).getIduser()).getCaminhoImg()%>" alt="" class="fotouser"> </a>	<!--COLOCAR IMAGEM DO USUARIO e LIGAR AO PERFIL DO USUARIO-->
                    <%}else{ %>
                    	<a href="RedirecionarServlet?action=perfil&id=<%= EspecialistaDao.buscarEspecialistaPorId(posts.get(i).getIduser()).getId()%>"> <img src="imagens/userpadrao.png" alt="" class="fotouser"> </a>
                    <%} %>  
                                <a href="RedirecionarServlet?action=perfil&id=<%= EspecialistaDao.buscarEspecialistaPorId(posts.get(i).getIduser()).getId()%>"><p><%= EspecialistaDao.buscarEspecialistaPorId(posts.get(i).getIduser()).getNome() %></p></a>       <!--COLOCAR NOME DO USUARIO (ESPECIALISTA.GETNOME()) e LIGAR AO PERFIL DO USUARIO-->
                            </div>
                            <span>•</span>
                            
                            <%String tema = ""; %>
                            <%if(posts.get(i).getTemaPredominante().equals("Inclusão Educacional")){ %>
                            	<%tema = "inclusao"; %>
                           <% }else if(posts.get(i).getTemaPredominante().equals("Acessibilidade")){ %>
                            	<%tema = "acessibilidade"; %>
                           <% }else if(posts.get(i).getTemaPredominante().equals("Deficiência Visual")){ %>
                            	<%tema = "deficiencia"; %>
                           <% }else if(posts.get(i).getTemaPredominante().equals("Autismo")){ %>
                            	<%tema = "autismo"; %>
                           <%} %>
                            
                           
                            <a href="RedirecionarServlet?action=<%=tema %>" style="color: #246CE0; font-weight: bold; text-decoration: none;"><span><%= posts.get(i).getTemaPredominante() %></span></a>    <!--COLOCAR O TEMA DO POST e LIGAR A PAGINA DO TEMA ESPECIFICO-->
                        </div>
